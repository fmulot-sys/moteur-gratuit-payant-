...
INSTRUCTION IMPORTANTE : L'utilisateur a renseigné des critères (type de sport, exclusivité, transversalité) mais tu dois TOUJOURS analyser le texte de l'article pour vérifier si ces critères sont cohérents avec le contenu réel. Si tu détectes une incohérence entre ce que l'utilisateur a déclaré et ce que le texte révèle, signale-le explicitement dans le champ "incoherences" et base ton verdict sur ta lecture du texte plutôt que sur les cases cochées.
...
Vérifie dans le texte si le sport traité correspond bien à la catégorie déclarée.
Top sports = football, rugby, tennis, cyclisme, Formule 1, NBA → aller à l'étape 2A
Autres sports = tous les autres sports → aller à l'étape 2B
Respire = contenu lifestyle / bien-être → aller à l'étape 2C
...
Règle importante sur les déclarations terrain : si le journaliste est présent sur place (zone mixte, conférence de presse, bord terrain), les déclarations sont considérées comme exclusives par défaut, même si d'autres médias sont potentiellement présents. On ne peut pas savoir combien de journalistes recueillent les mêmes propos. En cas de doute sur la présence terrain, demander la précision au journaliste plutôt que de déclasser en non-exclusif.
Vérifie dans le texte si la présence terrain est suggérée (mentions de zone mixte, vestiaires, bord terrain, conférence de presse, etc.).
→ SI OUI : PAYANT
→ SI NON : GRATUIT
...
Vérifie dans le texte si la transversalité déclarée est réelle.
...
### Cas limites
Si les critères sont ambigus ou se contredisent, signale-le explicitement dans "cas_limite".
...
Le champ "incoherences" doit signaler si le texte contredit les critères déclarés par l'utilisateur. Null si tout est cohérent.`;
        ...
        {view === "result" && result && (() => {
          ...
          return <div>
            <div style={{ background: cfg.bg, border: `2px solid ${cfg.border}`, borderRadius: 12, padding: "28px", marginBottom: 24 }}>
              ...
              {result.incoherences && <div style={{ marginTop: 16, background: "#fff0f0", border: "1px solid #c8102e", borderRadius: 8, padding: "12px 14px" }}>
                <span style={{ color: "#c8102e", fontSize: 13, fontWeight: 600 }}>⚠️ Incohérence détectée : </span>
                <span style={{ color: "#555", fontSize: 13 }}>{result.incoherences}</span>
              </div>}
              {result.cas_limite && <div style={{ marginTop: 12, background: "#fff8e0", border: "1px solid #f4a100", borderRadius: 8, padding: "12px 14px" }}>
                <span style={{ color: "#b45000", fontSize: 13, fontWeight: 600 }}>🔎 Cas limite : </span>
